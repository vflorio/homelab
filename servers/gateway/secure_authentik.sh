...
if sudo docker exec -e PGPASSWORD="$POSTGRES_PASSWORD" "$POSTGRESQL_CONTAINER" \
    psql -U "$POSTGRES_USER" -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname = '$AUTHENTIK_DATABASE';" | grep -q 1; then
...
sudo docker exec -e PGPASSWORD="$ADMIN_PASSWORD" -i "$POSTGRESQL_CONTAINER" psql -U "$POSTGRES_USER" -d "$AUTHENTIK_DATABASE" <<EOF
DO \$\$
BEGIN
...
GRANT CONNECT ON DATABASE "$AUTHENTIK_DATABASE" TO "$POSTGRES_USER";
